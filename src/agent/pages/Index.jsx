...
import { StatCard } from '@agent/components/dashboard/StatCard';
import { BookingsTable } from '@agent/components/dashboard/BookingsTable';
import { RevenueChart } from '@agent/components/dashboard/RevenueChart';
import { TripStats } from '@agent/components/dashboard/TripStats';
import { VehicleDriverQuickView } from '@agent/components/dashboard/VehicleDriverQuickView';
import { ReviewCard } from '@agent/components/dashboard/ReviewCard';
/* Shared UI Components */
import { Button } from '@agent/components/ui/button';
import { Link } from 'react-router-dom';
import { api, getAgentId } from '@agent/lib/api';
    ...
    const fetchData = async () => {
      try {
        const [statsRes, reviewsRes] = await Promise.all([
          fetch(`http://localhost:8082/api/v1/agent/${getAgentId()}/dashboard/stats`),
          fetch(`http://localhost:8082/api/v1/agent/${getAgentId()}/reviews`),
        ]);
        ...
        if (statsRes.ok) {
          ...
          console.error(`Dashboard stats API error: ${statsRes.status}`);
          setStats({});
        }
        ...
        if (reviewsRes.ok) {
          ...
          console.error(`Reviews API error: ${reviewsRes.status}`);
        }
      } catch (error) {
    ...
    >
      <div className="space-y-6">
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">
          {loading ? (
            <p className="text-muted-foreground col-span-6">Loading stats...</p>
          ) : (
