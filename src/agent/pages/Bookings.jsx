...
} from '@agent/components/ui/alert-dialog';
import {
  ...
  const invoiceHTML = `
    ...
    <div class="section">
      ...
      <div class="payment-box">
        ...
        <div class="amount">$${(booking.totalPrice || 0).toLocaleString()}</div></div>
      ...
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            <span className={cn(
              'ml-auto inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium capitalize',
              ...
              <div className="flex items-center gap-6 text-sm">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="font-medium">${(booking.totalPrice || 0).toLocaleString()}</p>
                  </div>
                </div>
              </div>
            </div>
        ...
        <DialogFooter className="gap-2">
          {booking.status === 'completed' && (
            <Button variant="outline" className="gap-1.5" onClick={() => { onClose(); handleDownloadInvoice(booking); }}>
...
const Bookings = () => {
  ...
  const renderActionButtons = (booking) => {
    ...
    // ── PENDING: new request awaiting agent decision ──────────────────────────
    if (status === 'pending') {
      return (
        <div className="flex flex-wrap gap-2">
          ...
          <Button size="sm" variant="outline"
            ...
            onClick={() => handleOpenAcceptModal(booking.id)}>
            ...
            {filteredBookings.map(booking => {
              ...
              return (
                <div key={booking.id}
                  ...
                  {/* Card header: avatar + package name + booking ID + status badge */}
                  ...
                  <div className="mt-4 grid gap-2 sm:grid-cols-2">
                    {booking.destination && (
                      <div className="flex items-center gap-2 text-sm">
                        <MapPin className="h-4 w-4 shrink-0 text-muted-foreground" />
                        <span className="text-foreground truncate">{booking.destination}</span>
                      </div>
                    )}
