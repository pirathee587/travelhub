...
export default function AdminProfileDialog() {
  ...
  const toast = {
    success: (msg) => modalContext ? modalContext.addToast(`✅ ${msg}`) : alert(msg),
    error: (msg) => modalContext ? modalContext.addToast(`❌ ${msg}`) : alert(msg)
  };
  ...
  const [draftAvatar, setDraftAvatar] = useState(profile.avatarUrl);
  ...
  function handleImageUpload(e) {
    ...
    reader.onload = (ev) => setDraftAvatar(ev.target?.result);
  ...
  const strengthIndex = ["Weak", "Fair", "Good", "Strong"].indexOf(strength?.label) + 1;
                        ...
                        <div className="flex gap-1.5">
                          {[1, 2, 3, 4].map((i) => (
                            <div
                              ...
                              style={{
                                background: i <= strengthIndex ? strength?.color : "#e5e7eb",
                              }}
                            />
                          ))}
                        </div>
