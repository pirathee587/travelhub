...
import { MapPin, Compass, Star, ChevronRight, Loader2 } from "lucide-react";
...
import { usePackagesByDistrict } from "@/features/tourist/hooks/useApi";

interface SriLankaTravelMapProps {
    selectedDistrict?: string;
    onSelectDistrict?: (district: string) => void;
    districtsWithPackages?: string[];
    packageCountMap?: Record<string, number>;
}

// Center coordinate centroids for all 25 districts in the SVG coordinate space (450x793)
const DISTRICTS_INFO: Record<string, { name: string; x: number; y: number }> = {
    colombo: { name: "Colombo", x: 60, y: 590 },
    gampaha: { name: "Gampaha", x: 80, y: 520 },
    kalutara: { name: "Kalutara", x: 75, y: 650 },
    kandy: { name: "Kandy", x: 190, y: 520 },
    matale: { name: "Matale", x: 190, y: 410 },
    "nuwara-eliya": { name: "Nuwara Eliya", x: 195, y: 580 },
    galle: { name: "Galle", x: 100, y: 720 },
    matara: { name: "Matara", x: 160, y: 745 },
    hambantota: { name: "Hambantota", x: 280, y: 710 },
    jaffna: { name: "Jaffna", x: 100, y: 60 },
    kilinochchi: { name: "Kilinochchi", x: 110, y: 115 },
    mannar: { name: "Mannar", x: 75, y: 190 },
    vavuniya: { name: "Vavuniya", x: 150, y: 200 },
    mullaitivu: { name: "Mullaitivu", x: 195, y: 150 },
    batticaloa: { name: "Batticaloa", x: 330, y: 390 },
    ampara: { name: "Ampara", x: 315, y: 510 },
    trincomalee: { name: "Trincomalee", x: 260, y: 240 },
    kurunegala: { name: "Kurunegala", x: 110, y: 385 },
    puttalam: { name: "Puttalam", x: 60, y: 310 },
    anuradhapura: { name: "Anuradhapura", x: 175, y: 270 },
    polonnaruwa: { name: "Polonnaruwa", x: 260, y: 320 },
    badulla: { name: "Badulla", x: 250, y: 550 },
    moneragala: { name: "Monaragala", x: 290, y: 600 },
    kegalle: { name: "Kegalle", x: 135, y: 510 },
    ratnapura: { name: "Ratnapura", x: 150, y: 620 }
};

export function SriLankaTravelMap({
    selectedDistrict: propSelectedDistrict,
    onSelectDistrict,
    districtsWithPackages = [],
    packageCountMap = {}
}: SriLankaTravelMapProps) {
    const navigate = useNavigate();
    const { formatPrice } = useTouristCurrency();
    
    // Internal state if used as standalone component (Overview page)
    const [internalSelected, setInternalSelected] = useState<string>("Colombo");
    
    const activeDistrict = propSelectedDistrict !== undefined ? propSelectedDistrict : internalSelected;

    const handleSelect = (districtName: string) => {
        if (onSelectDistrict) {
            onSelectDistrict(districtName);
        } else {
            setInternalSelected(districtName);
        }
    };

    // Find current active district key for visual effects
    const activeKey = useMemo(() => {
        return Object.keys(DISTRICTS_INFO).find(
            key => DISTRICTS_INFO[key].name.toLowerCase() === activeDistrict?.toLowerCase()
        ) || "colombo";
    }, [activeDistrict]);

    const activeInfo = DISTRICTS_INFO[activeKey];

    // Fetch packages for active district
    const { data: districtPackages, isLoading: packagesLoading } = usePackagesByDistrict(activeInfo?.name);

    // Filter and sort packages (highly rated 4.0+ first, limit to 5)
    const filteredAndSortedPackages = useMemo(() => {
        if (!districtPackages) return [];
        return districtPackages
            .filter((pkg: any) => {
                const rating = pkg.rating !== null && pkg.rating !== undefined ? pkg.rating : 5.0;
                return rating >= 4.0;
            })
            .sort((a: any, b: any) => {
                const ratingA = a.rating !== null && a.rating !== undefined ? a.rating : 5.0;
                const ratingB = b.rating !== null && b.rating !== undefined ? b.rating : 5.0;
                return ratingB - ratingA;
            })
            .slice(0, 5);
    }, [districtPackages]);

    // Helper to determine if a district name maps to package data
    const getPackageCount = (districtName: string) => {
        return packageCountMap[districtName] || (districtsWithPackages.includes(districtName) ? 1 : 0);
    };
    return (
        <div className="flex flex-col gap-6">
            <div className="grid grid-cols-1 md:grid-cols-12 gap-6 items-stretch">
                
                {/* SVG Interactive Map Area (7 cols) */}
                <div className="md:col-span-7 bg-gradient-to-br from-emerald-500/5 via-blue-500/5 to-primary/5 dark:from-slate-900/60 dark:to-slate-800/40 rounded-3xl border border-border/80 p-6 relative overflow-hidden flex items-center justify-center min-h-[500px]">
                    <div className="absolute inset-0 bg-[radial-gradient(#059669_1px,transparent_1px)] [background-size:24px_24px] opacity-10" />
                    
                    {/* SVG Map Container */}
                    <div className="relative w-full max-w-[340px] sm:max-w-[380px] aspect-[450/793] my-auto">
                        <svg
                            viewBox="0 0 450 793"
                        ...
                        >
                            ...
                            <path
                                ...
                                className={cn(
                                    "cursor-pointer transition-all duration-300 stroke-emerald-600/30 stroke-1 hover:fill-primary/20",
                                    activeKey === "colombo" ? "fill-primary/30 stroke-primary stroke-2" : "fill-emerald-100/40 dark:fill-slate-800/40"
                                )}
                            ...
                            <path
                                id="gampaha"
                                onClick={() => handleSelect("Gampaha")}
                                className={cn(
                                    "cursor-pointer transition-all duration-300 stroke-emerald-600/30 stroke-1 hover:fill-primary/20",
                                    activeKey === "gampaha" ? "fill-primary/30 stroke-primary stroke-2" : "fill-emerald-100/40 dark:fill-slate-800/40"
                                )}
                            ...
                            <path
                                id="kalutara"
                                onClick={() => handleSelect("Kalutara")}
                                className={cn(
                                    "cursor-pointer transition-all duration-300 stroke-emerald-600/30 stroke-1 hover:fill-primary/20",
                                    activeKey === "kalutara" ? "fill-primary/30 stroke-primary stroke-2" : "fill-emerald-100/40 dark:fill-slate-800/40"
                                )}
                            ...
                            <path
                                id="kandy"
                                onClick={() => handleSelect("Kandy")}
                                className={cn(
                                    "cursor-pointer transition-all duration-300 stroke-emerald-600/30 stroke-1 hover:fill-primary/20",
                                    activeKey === "kandy" ? "fill-primary/30 stroke-primary stroke-2" : "fill-emerald-100/40 dark:fill-slate-800/40"
                                )}
                            ...
                            <path
                                id="matale"
                                onClick={() => handleSelect("Matale")}
                                className={cn(
                                    "cursor-pointer transition-all duration-300 stroke-emerald-600/30 stroke-1 hover:fill-primary/20",
                                    activeKey === "matale" ? "fill-primary/30 stroke-primary stroke-2" : "fill-emerald-100/40 dark:fill-slate-800/40"
                                )}
                            ...
                            <path
                                id="nuwara-eliya"
                                onClick={() => handleSelect("Nuwara Eliya")}
                                className={cn(
                                    "cursor-pointer transition-all duration-300 stroke-emerald-600/30 stroke-1 hover:fill-primary/20",
                                    activeKey === "nuwara-eliya" ? "fill-primary/30 stroke-primary stroke-2" : "fill-emerald-100/40 dark:fill-slate-800/40"
                                )}
                            ...
                            <path
                                id="galle"
                                onClick={() => handleSelect("Galle")}
                                className={cn(
                                    "cursor-pointer transition-all duration-300 stroke-emerald-600/30 stroke-1 hover:fill-primary/20",
                                    activeKey === "galle" ? "fill-primary/30 stroke-primary stroke-2" : "fill-emerald-100/40 dark:fill-slate-800/40"
                                )}
                            ...
                            <path
                                id="matara"
                                onClick={() => handleSelect("Matara")}
                                className={cn(
                                    "cursor-pointer transition-all duration-300 stroke-emerald-600/30 stroke-1 hover:fill-primary/20",
                                    activeKey === "matara" ? "fill-primary/30 stroke-primary stroke-2" : "fill-emerald-100/40 dark:fill-slate-800/40"
                                )}
                            ...
                            <path
                                id="hambantota"
                                onClick={() => handleSelect("Hambantota")}
                                className={cn(
                                    "cursor-pointer transition-all duration-300 stroke-emerald-600/30 stroke-1 hover:fill-primary/20",
                                    activeKey === "hambantota" ? "fill-primary/30 stroke-primary stroke-2" : "fill-emerald-100/40 dark:fill-slate-800/40"
                                )}
                            ...
                            <path
                                id="jaffna"
                                onClick={() => handleSelect("Jaffna")}
                                className={cn(
                                    "cursor-pointer transition-all duration-300 stroke-emerald-600/30 stroke-1 hover:fill-primary/20",
                                    activeKey === "jaffna" ? "fill-primary/30 stroke-primary stroke-2" : "fill-emerald-100/40 dark:fill-slate-800/40"
                                )}
                            ...
                            <path
                                id="kilinochchi"
                                onClick={() => handleSelect("Kilinochchi")}
                                className={cn(
                                    "cursor-pointer transition-all duration-300 stroke-emerald-600/30 stroke-1 hover:fill-primary/20",
                                    activeKey === "kilinochchi" ? "fill-primary/30 stroke-primary stroke-2" : "fill-emerald-100/40 dark:fill-slate-800/40"
                                )}
                            ...
                            <path
                                id="mannar"
                                onClick={() => handleSelect("Mannar")}
                                className={cn(
                                    "cursor-pointer transition-all duration-300 stroke-emerald-600/30 stroke-1 hover:fill-primary/20",
                                    activeKey === "mannar" ? "fill-primary/30 stroke-primary stroke-2" : "fill-emerald-100/40 dark:fill-slate-800/40"
                                )}
                            ...
                            <path
                                id="vavuniya"
                                onClick={() => handleSelect("Vavuniya")}
                                className={cn(
                                    "cursor-pointer transition-all duration-300 stroke-emerald-600/30 stroke-1 hover:fill-primary/20",
                                    activeKey === "vavuniya" ? "fill-primary/30 stroke-primary stroke-2" : "fill-emerald-100/40 dark:fill-slate-800/40"
                                )}
                            ...
                            <path
                                id="mullaitivu"
                                onClick={() => handleSelect("Mullaitivu")}
                                className={cn(
                                    "cursor-pointer transition-all duration-300 stroke-emerald-600/30 stroke-1 hover:fill-primary/20",
                                    activeKey === "mullaitivu" ? "fill-primary/30 stroke-primary stroke-2" : "fill-emerald-100/40 dark:fill-slate-800/40"
                                )}
                            ...
                            <path
                                id="batticaloa"
                                onClick={() => handleSelect("Batticaloa")}
                                className={cn(
                                    "cursor-pointer transition-all duration-300 stroke-emerald-600/30 stroke-1 hover:fill-primary/20",
                                    activeKey === "batticaloa" ? "fill-primary/30 stroke-primary stroke-2" : "fill-emerald-100/40 dark:fill-slate-800/40"
                                )}
                            ...
                            <path
                                id="ampara"
                                onClick={() => handleSelect("Ampara")}
                                className={cn(
                                    "cursor-pointer transition-all duration-300 stroke-emerald-600/30 stroke-1 hover:fill-primary/20",
                                    activeKey === "ampara" ? "fill-primary/30 stroke-primary stroke-2" : "fill-emerald-100/40 dark:fill-slate-800/40"
                                )}
                            ...
                            <path
                                id="trincomalee"
                                onClick={() => handleSelect("Trincomalee")}
                                className={cn(
                                    "cursor-pointer transition-all duration-300 stroke-emerald-600/30 stroke-1 hover:fill-primary/20",
                                    activeKey === "trincomalee" ? "fill-primary/30 stroke-primary stroke-2" : "fill-emerald-100/40 dark:fill-slate-800/40"
                                )}
                            ...
                            <path
                                id="kurunegala"
                                onClick={() => handleSelect("Kurunegala")}
                                className={cn(
                                    "cursor-pointer transition-all duration-300 stroke-emerald-600/30 stroke-1 hover:fill-primary/20",
                                    activeKey === "kurunegala" ? "fill-primary/30 stroke-primary stroke-2" : "fill-emerald-100/40 dark:fill-slate-800/40"
                                )}
                            ...
                            <path
                                id="puttalam"
                                onClick={() => handleSelect("Puttalam")}
                                className={cn(
                                    "cursor-pointer transition-all duration-300 stroke-emerald-600/30 stroke-1 hover:fill-primary/20",
                                    activeKey === "puttalam" ? "fill-primary/30 stroke-primary stroke-2" : "fill-emerald-100/40 dark:fill-slate-800/40"
                                )}
                            ...
                            <path
                                id="anuradhapura"
                                onClick={() => handleSelect("Anuradhapura")}
                                className={cn(
                                    "cursor-pointer transition-all duration-300 stroke-emerald-600/30 stroke-1 hover:fill-primary/20",
                                    activeKey === "anuradhapura" ? "fill-primary/30 stroke-primary stroke-2" : "fill-emerald-100/40 dark:fill-slate-800/40"
                                )}
                            ...
                            <path
                                id="polonnaruwa"
                                onClick={() => handleSelect("Polonnaruwa")}
                                className={cn(
                                    "cursor-pointer transition-all duration-300 stroke-emerald-600/30 stroke-1 hover:fill-primary/20",
                                    activeKey === "polonnaruwa" ? "fill-primary/30 stroke-primary stroke-2" : "fill-emerald-100/40 dark:fill-slate-800/40"
                                )}
                            ...
                            <path
                                id="badulla"
                                onClick={() => handleSelect("Badulla")}
                                className={cn(
                                    "cursor-pointer transition-all duration-300 stroke-emerald-600/30 stroke-1 hover:fill-primary/20",
                                    activeKey === "badulla" ? "fill-primary/30 stroke-primary stroke-2" : "fill-emerald-100/40 dark:fill-slate-800/40"
                                )}
                            ...
                            <path
                                id="moneragala"
                                onClick={() => handleSelect("Monaragala")}
                                className={cn(
                                    "cursor-pointer transition-all duration-300 stroke-emerald-600/30 stroke-1 hover:fill-primary/20",
                                    activeKey === "moneragala" ? "fill-primary/30 stroke-primary stroke-2" : "fill-emerald-100/40 dark:fill-slate-800/40"
                                )}
                        ...
                        {Object.keys(DISTRICTS_INFO).map((key) => {
                            ...
                            return (
                                ...
                                >
                                    {/* Glowing Pulse for selected/hovered active district */}
                                    <span className={cn(
                                        "absolute -inset-3 rounded-full bg-primary/20 transition-all duration-300 scale-75 group-hover:scale-100 group-hover:bg-primary/30",
                                        isSelected ? "scale-110 bg-primary/30 animate-pulse" : ""
                                    )} />
                                    
                                    {/* Standard Pin Maker */}
                                    <div
                                        className={cn(
                                            "relative flex items-center justify-center rounded-full transition-all duration-300 shadow-md h-7 w-7",
                                            isSelected
                                                ? "bg-primary text-white scale-125 ring-4 ring-primary/30 z-30"
                                                : "bg-background text-primary border border-primary/40 hover:bg-primary/10 hover:scale-110"
                                        )}
                                    >
                                        <MapPin className={cn("h-4 w-4", isSelected && "animate-bounce")} />
                                    </div>

                                    {/* Tooltip Badge */}
                                    <div
                                        className={cn(
                                            "absolute top-full left-1/2 -translate-x-1/2 mt-1 px-2 py-0.5 rounded-md text-[10px] font-bold tracking-tight whitespace-nowrap shadow-sm transition-all pointer-events-none",
                                            isSelected
                                                ? "bg-primary text-white scale-105 z-30"
                                                : "bg-background/90 text-foreground border border-border opacity-70 group-hover:opacity-100 group-hover:scale-105"
                                        )}
                                    >
                                        {info.name}
                                    </div>
                ...
                {/* Right Side: Selected Destination Inspector Card (5 cols) */}
                <div className="md:col-span-5 flex flex-col justify-between bg-card rounded-3xl border border-border/80 p-6 overflow-hidden min-h-[500px]">
                    <div className="flex flex-col gap-5 flex-1 min-h-0">
                        <div>
                            <div className="flex items-center gap-2 mb-2 text-xs text-muted-foreground">
                                <Compass className="h-4 w-4 text-primary" />
                                <span>District Destination Focus</span>
                            </div>
                            <h3 className="text-2xl font-black text-foreground flex items-center gap-2">
                                ...
                                <Badge variant="outline" className="bg-primary/10 text-primary border-primary/20 hover:bg-primary/20">
                                    {getPackageCount(activeInfo.name)} packages
                                </Badge>
                            </h3>
                        </div>
                        ...
                        <div className="space-y-3 flex-1 flex flex-col min-h-0">
                            ...
                            <div className="flex-1 overflow-y-auto pr-1 max-h-[340px]">
                                {packagesLoading ? (
                                    ...
                                    <div className="flex flex-col gap-2.5">
                                        {filteredAndSortedPackages.map((pkg: any) => (
                                            <div
                                                key={pkg.id}
                                                onClick={() => navigate(`/tourist/explore/package/${pkg.id}`)}
                                                className="group/item flex gap-3 p-2 rounded-xl bg-secondary/30 hover:bg-secondary/70 border border-border/40 hover:border-primary/30 transition-all cursor-pointer"
                                            >
                                                {/* Mini Package Image */}
                                                ...
                                                {/* Mini Package Details */}
                                                <div className="flex-1 min-w-0 flex flex-col justify-between py-0.5">
                                                    <h5 className="font-bold text-xs text-foreground truncate group-hover/item:text-primary transition-colors">
                                                        {pkg.packageName}
                                                    </h5>
                                    ...
                                    <div className="text-center py-12 text-muted-foreground border border-dashed rounded-xl bg-secondary/10">
                                        <MapPin className="h-8 w-8 mx-auto mb-2 opacity-40" />
                                        <p className="text-xs">No active packages in {activeInfo.name}</p>
                                    </div>
                                )}
                            </div>
                        </div>
                    ...
                    <div className="mt-8 pt-4 border-t border-border/50 text-xs text-muted-foreground flex items-center justify-between">
                        <span>Click map districts or pins to explore</span>
