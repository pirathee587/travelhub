...
public class ReceiptService {
    ...
    /**
     * Generates a digital payment invoice for a specific booking.
     * Constructs a multi-section PDF containing customer details and a booking summary.
    ...
    public byte[] generateBookingInvoice(Long bookingId) throws Exception {
        ...
        PdfPCell subTitleCell = new PdfPCell(new Phrase("OFFICIAL BOOKING INVOICE", subTitleFont));
        ...
        // ── Stakeholder & Invoice Information ───────────────
        ...
        addTableCell(infoTable, "INVOICE DETAILS", boldFont, true);

        // Customer Row
        addTableCell(infoTable, "Name: " + booking.getUser().getName(), normalFont, false);
        addTableCell(infoTable, "Invoice No: INV-" + booking.getId() + "-" + Math.abs(booking.getCreatedAt().hashCode()), normalFont, false);

        // Contact Row
        addTableCell(infoTable, "Email: " + booking.getUser().getEmail(), normalFont, false);
        addTableCell(infoTable, "Date: " + booking.getCreatedAt().format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm")), normalFont, false);

        // Status Row
        addTableCell(infoTable, "Phone: " + (booking.getUser().getTelephone() != null ? booking.getUser().getTelephone() : "N/A"), normalFont, false);
        addTableCell(infoTable, "Status: " + booking.getStatus().toUpperCase(), boldFont, false);
        ...
        PdfPCell totalLabel = new PdfPCell(new Phrase("TOTAL DUE (USD)", boldFont));
        totalLabel.setPadding(10f);
        totalLabel.setHorizontalAlignment(Element.ALIGN_RIGHT);
        summaryTable.addCell(totalLabel);

        PdfPCell totalValue = new PdfPCell(new Phrase(String.format("%,.2f", booking.getTotalPrice()), boldFont));
